...
insert_stmt = "insert into #{@['db_database']}.instance_snapshot(instance_id,max_processes,active_processes,process_count,requests_in_queues,global_queue,free_memory,date_time) values('#{instance_id}',#{passenger_status[:max]},#{passenger_status[:active]},#{passenger_status[:count]},#{passenger_status[:requests_in_queues]},#{passenger_status[:global_queue]},#{free_memory},now)"
